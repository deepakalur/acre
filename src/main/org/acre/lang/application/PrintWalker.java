...
/**
 * Simple OQL AST walker. This is simple OQL Abstract Syntax Tree walker
 ...
 * testing the tree structure for a given OQL sentence.
 *
 * @author Mariusz Nowostawski
 * @version @version@ $Revision: 1.1.1.1 $
 */
class PrintWalker extends DepthFirstAdapter {
    ...
    public void defaultCase(Node node) {
        indent();
        String className = node.getClass().toString();
        System.out.println(className.substring(className.lastIndexOf('.') + 1) + ": " + ((Token) node).getText());
    }
